---
title: "Prognosemodel - `r params$uitval` `r ifelse(params$propedeusediploma == 'Nvt', '', paste0(' - ', params$propedeusediploma))`"
...
if(sPropedeusediploma == "Zonder P") {
  sUitval_model_text <- paste(sUitval_model, "bij studenten zonder propedeusediploma")
} else if(sPropedeusediploma == "Met P") {
  sUitval_model_text <- paste(sUitval_model, "bij studenten met propedeusediploma")
} else {
  sUitval_model_text <- sUitval_model
}
...
We maken een explainer om de voorspellingen van het model te kunnen verklaren. De explainer geeft inzicht in de voorspellingen van het model en de bijdrage van de variabelen aan de voorspelling.
...
# Maak een explainer
explain_lm <- DALEX::explain(model = workflow,
...
Een prognosemodel moet minimaal beter presteren dan een *base-model* om waarde toe te voegen. Het base-model neemt de grootste klasse van de gemiddelde `r tolower(sUitval_model)` van de afgelopen jaren als basis. Stel we zouden tegen alle studenten zeggen dat ze hun studie gaan halen, dan is de mate van accuratesse gelijk aan dit base-model. Dit base-model is dus altijd hoger dan de 50% lijn van de AUC/ROC curve, tenzij het base-model toevallig precies 50% is
...
Theo Bakker, Lectoraat Learning Technology & Analytics, De Haagse Hogeschool © 2023-`r nCurrent_year` Alle rechten voorbehouden.
:::
