...
De basis voor deze analyse is studiedata van De Haagse Hogeschool (De HHs), verrijkt door het lectoraat LTA. De data bevat informatie over de inschrijvingen van studenten in het eerste jaar van de sp:
...
get_tbl_data_dictionary(dfData_dictionary)
...
-   Retentie is gedefinieerd als ingeschreven staan in dezelfde sp in een aansluitend collegejaar. Een wisseling van sp_form binnen de sp, bijvoorbeeld van voltijd in jaar 1 naar duaal in jaar 2, geldt ook als retentie. 
-   Uitval is het tegenovergestelde van retentie: niet ingeschreven staan in dezelfde sp in een aansluitend collegejaar. 
-   Een wisseling van sp_form binnen de sp, bijvoorbeeld van voltijd in jaar 1 naar duaal in jaar 2, geldt *niet* als uitval.
...
if (params$use_synthetic_data) {
  df_sp_enrollments <- get_sp_enrollments_syn(
    sp = params$sp,
    sp_form = toupper(params$sp_abbreviation)) |> 
    mutate(
      ID = as.character(ID),
  ...
  df_sp_enrollments <- get_lta_studyprogram_enrollments_pin(
    board = "HHs/Inschrijvingen",
    faculty = params$faculty,
    studyprogram = params$study_programmesnaam,
    sp = params$sp,
    sp_form = toupper(params$study_programmesvorm),
    range = "eerstejaars")
}
...
df_sp_enrollments <- df_sp_enrollments |> 
  ...
  mutate(across(all_of(names(levels_formal)), ~ factor(.x, 
...
lName_mapping <- setNames(df_variables$VAR_Simple_variable, df_variables$VAR_Formal_variable)
...
tbl_dfSummary <- get_tbl_summary(df_summary) 
...
Clusters in correlaties kunnen per sp verschillen. Onderzoek in de correlaties hoe sterk deze zijn, welke clusters gevormd worden en of deze -- vanuit de context van de sp, faculty of onderwijinstitution -- logisch zijn. Verwerk de inzichten eventueel in een oplegger. Gangbare clusters zijn:
...
dfCorrelation <- df_sp_enrollments |> 
  select(-Collegejaar) |>
...
sBest_model_auc <- round(df_model_results$auc[df_model_results$best == TRUE], 4)
```
...
sText_best_model <- df_model_results |> 
  filter(best == FALSE) |> 
...
Last_fit_Accuracy   <- last_fit |>
  ...
  pull(.estimate) |>
  round(4) * 100
Avg_Retentie        <- round(mean(df_sp_enrollments$Retentie == TRUE) * 100, 2)
...
if (!("Eindcijfer_VO" %in% dfTop_vi$Variable[1:3]) 
  ...
  sConclusie_text <- glue::glue(
    sConclusie_text,
    "**Het model toont wel aan dat de herkomst van studenten sterker is gecorreleerd met {tolower(succes_model)} dan eerdere prestaties of vooropleiding.** ",
  ...
  sConclusie_text <- glue::glue(
    sConclusie_text,
    "**Het model toont aan dat eerdere prestaties van studenten sterker zijn gecorreleerd met {tolower(succes_model)} dan herkomst.** ",
    "De _Variable Importance_ (VI) laat namelijk zien dat de variabelen {lTop3_vi} de 3 belangrijkste variabelen zijn voor het voorspellen van {tolower(succes_model)}, gevolgd door {lTop45_vi}."
  )
} else {
...
### Het beste prognosemodel voor deze sp

**Het beste prognosemodel blijkt het `r sBest_model` model te zijn.**

-   Van de prognosemodellen die we hebben ontwikkeld om `r tolower(succes_model)` te voorspellen, had het `r sBest_model` model de hoogste AUC/ROC waarde (`r sBest_model_auc`).
...
-   **advanced-report: `r Base_Model_Accuracy`%** -- Voor deze sp berekenen we het advanced-report als volgt. Van alle studenten studeerde `r Avg_Retentie`% door; 100% - `r Avg_Retentie`% = `r Avg_Non_Retentie`% studeerde *niet* door. De grootste klasse van deze twee, `r Base_Model_Accuracy`%, is daarmee de accuratesse van het advanced-report.
-   **Accuratesse prognose: `r Last_fit_Accuracy`%** -- Het model voorspelt `r succes_model` met een accuratesse van `r Last_fit_Accuracy`%.
-   **Lift: `r pAccuracy_verschil`** -- Het model scoort in de huidige opbouw met een verschil van `r pAccuracy_verschil` (de *lift*) `r sAccuracy_verschil` `r sAccuracy_vergelijking` dan de accuraatheid van het advanced-report.
...
pTP  <- change_number_marks((df_confusion_matrix$n[4]/sum(df_confusion_matrix$n)*100),1)
pFP  <- change_number_marks((df_confusion_matrix$n[2]/sum(df_confusion_matrix$n)*100),1)
pTN  <- change_number_marks((df_confusion_matrix$n[1]/sum(df_confusion_matrix$n)*100),1)
pFN  <- change_number_marks((df_confusion_matrix$n[3]/sum(df_confusion_matrix$n)*100),1)
