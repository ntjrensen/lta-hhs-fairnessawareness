...
if(!file.exists(sPlotPath) | params$recreateplots == TRUE) {
  ...
  ## Bouw de basisplot
  plot <- suppressWarnings(plot(breakdown_lf_all, plot_distributions = TRUE)) 
  ...
  suppressWarnings(
    Finalize_Plot(
          plot_name = plot,
          save_filepath = sPlotPath,
          height_pixels = 50 + (15 * nrow(dfBreakdown_lf_all))
        )
    )
...
for (group in names(lDfPersona)) {
  ...
  Knit_Header(glue("Naar {tolower(group)}"), 3)
  ...
  for (j in 1:nrow(dfPersona)) {
    ...
    if(params$recreateplots) {
      ...
      plot <- Get_Waterfall_Plot(dfBreakdown_lf, lTitles)
      ...
      suppressWarnings(
        Finalize_Plot(
          plot_name = plot,
          save_filepath = sPlotPath,
          height_pixels = 50 + (15 * nrow(dfBreakdown_lf))
        ))
