...
if(!file.exists(sPlotPath) | params$recreateplots == TRUE) {
  ...
  suppressWarnings(
    Finalize_Plot(
          plot_name = plot,
          save_filepath = sPlotPath,
          height_pixels = 50 + (15 * nrow(dfBreakdown_lm_all))
        )
    )
    ...
    if(params$recreateplots) {
      ...
      suppressWarnings(
        Finalize_Plot(
          plot_name = plot,
          save_filepath = sPlotPath,
          height_pixels = 50 + (15 * nrow(dfBreakdown_lm))
        ))
...
if(!file.exists(sPlotPath) | params$recreateplots == TRUE) {
  ...
  suppressWarnings(
    Finalize_Plot(
      ...
      save_filepath = sPlotPath,
      height_pixels = 50 + (15 * length(unique(dfShapley$variable_name)))
    ))
...
Tot slot gaan we een aantal scenario's bekijken (wat als...). We nemen opnieuw de meest voorkomende studenten, maar beelden nu af hoe de kans op retentie eruit zou zien als telkens een van de variabelen net wat anders was geweest. 

:::{.callout-warning appearance='default' icon=true}
## Let op!
Dit is de invloed van de variabelen bij de unieke combinatie van deze meest voorkomende student per categorie. Zie voor de invloed van een variabelen ongeacht de combinatie de Partial Dependence Profielen.
:::
...
for (name in c("Geslacht", "Vooropleiding", "Aansluiting")) {
  ...
  if(params$recreateplots) {
    ...
    ## Bewaar de plot
    sPlotPath <- file.path(Get_Plot_outputpath(plotname = paste0("lf_cp_", tolower(name))))
