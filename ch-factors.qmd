...
if (!file.exists(sPlotPath) | params$recreateplots == TRUE) {
  ...
  dfBreakdown_lf_all <- get_df_breakdown_lf(breakdown_lf_all)
  
  # Define the titles
  titles_list <- get_breakdown_titles(breakdown_lf_all, dfPersona_all, 1, 
...
for (group in names(lDfPersona)) {
  ...
  # Walk the personas
  for (j in 1:nrow(df_persona)) {
    
    # Determine the current student
    student_current   <- df_persona[j, ]
    student_group     <- student_current$Groep
    student_category <- df_persona[j, ]$Categorie # levels(student_current[[student_group]])[j]
    
    # Determine the plot name
    sPlotPath <- get_breakdown_plotpath(student_group, student_category, bestmodel = sBest_model)
    ...
    if (!file.exists(sPlotPath) | params$recreateplots) {
      ...
      # Show the existing plot
      sPlot <- glue("![Breakdown naar {tolower(student_group)}: {student_category}]({sPlotPath}) {{#fig-breakdown-{gsub(' ', '-', tolower(student_group))}-{gsub(' ', '-', tolower(student_category))}}}")
