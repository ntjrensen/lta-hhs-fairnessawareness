...
params:
  ...
  # ## MO:ES-ES-3 VT
  # faculteit: "MO"
  # opleidingsnaam: "B European Studies"
  # opleiding: "ES-ES-3"
  ...
  # ## MO:FM DT
  # faculteit: "MO"
  # opleidingsnaam: "B Facility Management"
  # opleiding: "FM"
  # opleidingsvorm: "deeltijd"
  # opleidingsvorm_afkorting: "DT"
  # selectie: false
  ...
  ## TIS:CT-ES-3 VT
  # faculteit: "TIS"
  # opleidingsnaam: "B Chemische Technologie"
  # opleiding: "CT-ES-3"
  # opleidingsvorm: "voltijd"
  # opleidingsvorm_afkorting: "VT"
  # selectie: false
...
```{r}
#| label: amount_and_numbers_per_group
#| results: asis
#| echo: false

## Maak een tabel met aantallen en percentages voor de 3 variabelen `Geslacht`, `Vooropleiding` en `Aansluiting`
...
for(group in c("Geslacht", "Vooropleiding", "Aansluiting")) {
  ...
  dfFairness_check_data <- Get_dfFairness_Check_Data(fobject[["fairness_check_data"]])
...
Na de uitvoering van de fairness analyse kunnen we de volgende conclusies trekken:

```{r}
#| label: fairness_conclusions
#| results: asis
#| echo: false
...
## Maak een tabel van de fairness analyse
dfFairness_wide <- bind_rows(lDfFairness) |> 
  group_by(FRN_Group, 
           FRN_Subgroup, 
           FRN_Bias) |>
  summarise(FRN_Bias_count = n(), .groups = "drop") |> 
  pivot_wider(names_from = FRN_Bias, values_from = c(FRN_Bias_count)) |> 
  
  ## Vul missende waarden op met 0
  replace_na(list(`Geen Bias` = 0, 
                  `Negatieve Bias` = 0, 
                  `Positieve Bias` = 0)) |> 
  mutate(Bias = ifelse(`Negatieve Bias` > 1 | `Positieve Bias` > 1, 'Ja', 'Nee')) |> 
  ...
  mutate(Variabele = factor(Variabele, 
                            levels = c("Geslacht", "Vooropleiding", "Aansluiting"))) |> 
  arrange(Variabele, Groep)

# Create flextable
ft <- flextable(dfFairness_wide)

# Merge the 'Variabele' column for visual grouping
...
```
<!-- Dit in tekst automatiseren -->

1.  **Geslacht**: Er is geen sprake van bias in de voorspelling van retentie op basis van geslacht. De modelprestaties zijn even accuraat voor zowel mannen als vrouwen. De kansen op retentie zijn gelijk voor beide groepen.
2. **Vooropleiding**: Er is geen sprake van bias in de voorspelling van retentie op basis van vooropleiding. Het model is even accuraat voor zowel havisten als mbo-ers. De kansen op retentie zijn gelijk voor beide groepen.
3. **Aansluiting**: Er is geen sprake van bias in de voorspelling van retentie op basis van aansluiting. Het model is even accuraat voor zowel studenten die direct instromen als student.
