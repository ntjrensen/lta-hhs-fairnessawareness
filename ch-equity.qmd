...
for (group in c(sensitive_labels)) {
  ...
  sPlot <- glue("![Verdeling en dichtheid van {tolower(research_settings[['succes_label']])} naar  {tolower(group)}]({sPlotPath}) {{#fig-fairness-check-{tolower(group)}}}")
...
for (group in c(sensitive_labels)) {
  ...
  dfFairness_check_data <- get_df_fairness_check_data(fairness_object[["fairness_check_data"]], group = group)
