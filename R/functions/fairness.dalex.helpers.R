...
drop_metrics_with_na <- function(data) {
  ...
  if (any(na_col_index))
    warning("Found metric with NA: ",
            paste(cols_with_missing, collapse = ", "),
            ", omiting it\n")
...
assert_performance_metrics <- function(metric) {
  ...
  if (!metric %in% metrics)
    stop("Metric not in available metrics")
  invisible(return())
}
...
assert_different_label <- function(x) {
  labels <- unlist(lapply(x, function(x)
    x$label))
  
  if (length(labels) != length(unique(labels)))
    stop(
      "Some models have the same fairness labels, be sure to print/plot objects with different label "
    )
}
...
colors_fairmodels <- function(n = 2) {
  ...
  if (n == 8) {
    return(c(DALEX::colors_discrete_drwhy(n = 7), "#B622AD"))
  }
  if (n == 9) {
    return(c(DALEX::colors_discrete_drwhy(n = 7), "#B622AD", "#c295f0"))
  }
  if (n == 10) {
    return(c(
      DALEX::colors_discrete_drwhy(n = 7),
      "#B622AD",
      "#c295f0",
      "#037B63"
    ))
...
calculate_parity_loss <- function(gmm, privileged) {
  gmm_scaled <- apply(gmm, 2, function(x)
    x / gmm[, privileged])
  gmm_abs <- apply(gmm_scaled, 2, function(x)
    sapply(x, function(y)
      abs(log(y))))
  gmm_loss <- rowSums(gmm_abs)
  # when there is Inf in data change it to NA
  parity_loss <- sapply(gmm_loss, function(y)
...
check_fobjects <- function(fobjects, protected, privileged, verbose) {
  ...
  if (length(fobjects) > 0) {
    if (!all(sapply(fobjects, function(x)
      ...
      stop(
        "fairness objects must have the same protected vector as one passed in fairness check"
      )
    }
    if (!all(sapply(fobjects, function(x)
      ...
      stop(
        "fairness objects must have the same privlieged argument as one passed in fairness check"
      )
    }
    verbose_cat(
      "(",
...
check_explainers_clf <- function(all_explainers, protected, verbose) {
  if (!all(sapply(all_explainers, function(x)
    ...
    stop(
      "All models must be binary classification type. To check fairness in regression use 'fairness_check_regression()'"
    )
  }
...
check_explainers_reg <- function(all_explainers, protected, verbose) {
  if (!all(sapply(all_explainers, function(x)
    ...
    stop(
      "All models must be regression type. To check fairness in binary classification use 'fairness_check()'"
    )
  }
...
check_explainers <- function(all_explainers, protected, verbose) {
  ...
  if (!all(sapply(all_explainers, function(x)
    length(y_to_compare) == length(x$y)))) {
    verbose_cat("(",
                color_codes$red_start,
                "y not equal",
                color_codes$red_end,
...
check_labels <- function(label, explainers, fobjects_label) {
  if (is.null(label)) {
    label <- sapply(explainers, function(x)
      x$label)
  } else {
    if (length(label) != length(explainers))
      stop("Number of labels must be equal to number of explainers (outside fairness objects)")
  }
  
  # explainers must have unique labels
  if (length(unique(label)) != length(label)) {
    stop(
      "Explainers don't have unique labels
  ...
  if (length(unique(fobjects_label)) != length(fobjects_label)) {
    stop(
      "Fairness objects don't have unique labels
...
get_nice_ticks <- function(min_value, max_value, max_ticks = 9) {
  ...
  return(list(
    min = readable_minimum,
    max = readable_maximum,
