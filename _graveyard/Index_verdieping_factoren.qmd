---
title: "2. Factoranalyse - `r params$uitval` `r ifelse(params$propedeusediploma == 'Nvt', '', paste0(' - ', params$propedeusediploma))`"
...
3.  We gebruiken het beste model om de prognosemodellen te verklaren en te begrijpen. We kijken naar de bijdrage van de variabelen aan de voorspelling en passen het model toe op de meest voorkomende studenten.
...
if(!file.exists(sPlotPath) | params$recreateplots == TRUE) {
  ...
  ## Bouw de plot
  plot <- suppressWarnings(plot(breakdown_lm_all,
            plot_distributions = TRUE, 
            title = "Breakdown van de meest voorkomende student"
  ))
  ...
  print(glue("Toon de bestaande plot: {sPlotPath}"))
...
for (name in names(lDfPersona)) {
  ...
  ## Zo ja, laat dan buiten beschouwing
  ...
  for (j in 1:nrow(dfPersona)) {
    ...
    ## Bepaal de plotnaam
    sPlotPath <- file.path(Get_Plot_outputpath(plotname = tolower(glue("lf_break_down_{student_groep}_{student_categorie}"))))
      ...
      nPercentage <- Number_to_readable(as.numeric(dfPersona[j, 'Percentage']) * 100, digits = 1)
...
```{r, eval = !params$recreateplots}
#| label: lf_break_down_plots
#| echo: false
#| results: asis

## Als er te weinig observaties waren toon die
## TODO: dit maken

## Print de 3 categorieën

Get_Breakdown_Plots("Geslacht")
Get_Breakdown_Plots("Vooropleiding")
Get_Breakdown_Plots("Aansluiting")
