...
## Bepaal de lange naam van de opleidingsvorm en faculteit
...
In deze analyse onderzoeken we **`r sSucces_model`** voor de opleiding **`r params$opleidingsnaam`** (`r params$opleiding`), `r opleidingsvorm_lang`, eerstejaars, van de faculteit `r Get_Faculteitsnaam_Lang(params$faculteit)` (`r params$faculteit`).
...
Het lectoraat heeft een methode ontwikkeld voor de analyse van studiedata om te kunnen analyseren of er sprake is van bias in studiedata in relatie tot het succes van studenten, wat een indicatie kan zijn van een gebrek aan kansengelijkheid. Voor deze methode zetten we prognosemodellen in die we bouwen met machine learning. Een prognosemodel is dus niet een doel op zich, maar het instrument voor een analyse van kansengelijkheid, ook wel een *fairness* analyse genoemd. Zie voor een verdere toelichting op het onderzoeksprogramma: '[No Fairness without Awareness](https://www.dehaagsehogeschool.nl/onderzoek/kenniscentra/no-fairness-without-awareness)'.
